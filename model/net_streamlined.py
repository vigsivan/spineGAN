...
from model.layer import init_weights, PureUpsampling, ConfidenceDrivenMaskLayer3D, SpectralNorm
import numpy as np
...
class GMCNN(BaseNet):
    def __init__(self, in_channels, out_channels, cnum=32, act=F.elu, norm=F.instance_norm, using_norm=False):
        ...
        self.EB1.append(UpSample(spatial_dims=3, in_channels=ch*4, out_channels=ch*4, scale_factor=4))
        ...
        self.EB2.append(UpSample(spatial_dims=3, in_channels=ch*4, out_channels=ch*4, scale_factor=2))
        self.EB2.append(nn.Conv3d(ch * 4, ch * 2, kernel_size=5, stride=1))
        self.EB2.append(nn.Conv3d(ch * 2, ch * 2, kernel_size=5, stride=1))
        self.EB2.append(UpSample(spatial_dims=3, in_channels=ch*2, out_channels=ch*2, scale_factor=2))
        ...
        self.EB3.append(UpSample(spatial_dims=3, in_channels=ch*4, out_channels=ch*4, scale_factor=2))
        self.EB3.append(nn.Conv3d(ch * 4, ch * 2, kernel_size=3, stride=1))
        self.EB3.append(nn.Conv3d(ch * 2, ch * 2, kernel_size=3, stride=1))
        self.EB3.append(UpSample(spatial_dims=3, in_channels=ch*2, out_channels=ch*2, scale_factor=2))
        ...
        self.decoding_layers.append(nn.Conv3d(ch // 2, out_channels, kernel_size=3, stride=1))
        ...
        required_padding = list(set(self.EB1_pad_rec+self.EB2_pad_rec+self.EB3_pad_rec+self.decoding_pad_rec))
...
class Discriminator(BaseNet):
    def __init__(self, in_channels, cnum=32, fc_channels=8*8*32*4, act=F.elu, norm=None, spectral_norm=True):
        ...
        self.layers = []
        if spectral_norm:
            self.layers.append(SpectralNorm(nn.Conv3d(in_channels, ch, kernel_size=5, padding=2, stride=2)))
            self.layers.append(SpectralNorm(nn.Conv3d(ch, ch * 2, kernel_size=5, padding=2, stride=2)))
            self.layers.append(SpectralNorm(nn.Conv3d(ch * 2, ch * 4, kernel_size=5, padding=2, stride=2)))
            self.layers.append(SpectralNorm(nn.Conv3d(ch * 4, ch * 4, kernel_size=5, padding=2, stride=2)))
            self.layers.append(SpectralNorm(nn.Linear(fc_channels, 1)))
        else:
            self.layers.append(nn.Conv3d(in_channels, ch, kernel_size=5, padding=2, stride=2))
            self.layers.append(nn.Conv3d(ch, ch * 2, kernel_size=5, padding=2, stride=2))
            self.layers.append(nn.Conv3d(ch*2, ch*4, kernel_size=5, padding=2, stride=2))
            self.layers.append(nn.Conv3d(ch*4, ch*4, kernel_size=5, padding=2, stride=2))
...
class InpaintingModel_GMCNN(BaseModel):
    def __init__(self, in_channels, act=F.elu, norm=None, opt=None):
        ...
        self.netGM = GMCNN(in_channels, out_channels=1, cnum=opt.g_cnum, act=act, norm=norm).cuda()
        ...
        if self.opt.pretrain_network is False:
            ...
            if self.opt.mask_type == 'rect':
                self.netD = GlobalLocalDiscriminator(1, cnum=opt.d_cnum, act=act,
                ...
                self.netD = GlobalLocalDiscriminator(1, cnum=opt.d_cnum, act=act,
                                                     ...
                                                     g_fc_channels=g_fc_channels,
                                                     l_fc_channels=l_fc_channels).cuda()
    ...
    def initVariables(self):
        ...
        mask, rect = generate_mask3d(self.opt.img_shapes, self.opt.mask_shapes)
        self.mask_01 = einops.repeat(mask, 'd h w -> b c d h w', b=self.opt.batch_size, c=1)
        self.mask_01 = self.mask_01.cuda()
        self.mask = self.confidence_mask_layer(self.mask_01)
        self.rect = [rect[0,i] for i in range(6)]
    ...
    def forward_G(self):
        ...
        if self.opt.pretrain_network is False:
            ...
            # pred_slice = einops.repeat(pred_slice, 'b c h w -> b (repeat c) h w', repeat=3)
